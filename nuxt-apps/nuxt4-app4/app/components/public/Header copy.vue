...
const headerItems = [
  { name: "ទំព័រដើម", url: "/" },
  { name: "សេវាកម្ម", url: "/services" },
  { name: "អំពីយើង", url: "/about" },
];
...
const headerButtons = [
  {
    name: "ចូលគណនី",
    url: "/login",
    classes: "bg-transparent py-3 px-4 rounded-lg transition duration-200 hover:bg-gray-200",
  },
  {
    name: "ប្រើប្រាស់ឥឡូវនេះ",
    url: "/app/dashboard",
    classes: "text-white bg-public-red py-3 px-4 rounded-lg transition duration-200 hover:bg-public-red/60",
  },
];
</script>
...
<template>
  <header class="sticky top-0 left-0 right-0 z-50 w-full bg-zinc-100/70 shadow backdrop-blur-lg">
    <nav class="mx-auto max-w-7xl px-4 py-6 transition-all duration-300">
      ...
      <NuxtLink to="/" class="flex items-center">
        ...
        <ul class="hidden md:flex items-center gap-8 font-semibold text-gray-600">
          <li v-for="item in headerItems" :key="item.name">
            <NuxtLink :to="item.url" class="transition duration-200 hover:text-public-red">
              {{ item.name }}
            </NuxtLink>
          </li>
        </ul>
        ...
        <div v-if="isMenuOpen" class="md:hidden mt-4 pb-6 border-t border-gray-200">
          <ul class="flex flex-col gap-4 py-6 text-center">
            <li v-for="item in headerItems" :key="item.name">
              <NuxtLink :to="item.url" class="text-lg font-semibold text-zinc-900" @click="isMenuOpen = false">
                {{ item.name }}
              </NuxtLink>
            </li>
          </ul>
          <ul class="flex flex-col gap-4 text-center">
            <li v-for="b in headerButtons" :key="b.name">
              <NuxtLink :to="b.url" :class="['block w-full', b.classes]" @click="isMenuOpen = false">
                {{ b.name }}
              </NuxtLink>
            </li>
          </ul>
